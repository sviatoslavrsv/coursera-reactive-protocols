...
object SelectiveReceive {
   ...
   *           Use [[validateAsInitial]] to make sure that `initialBehavior` is a
   ...
   * @tparam T Type of messages
   *
   *           Hints: Use [[interpretMessage]] to compute the next behavior.
   *           Use [[isUnhandled]] to know if the message was unhandled by the behavior.
   *           Use [[canonicalize]] to make the next behavior applicable to the
